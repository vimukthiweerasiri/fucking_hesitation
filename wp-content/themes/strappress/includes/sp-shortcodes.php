...
function sp_buttons( $atts, $content = null ) {
    ...
    $out = '<a ' . $poptitle . '' . $popcontent . '' . $target. ' class="btn ' . $size . '' . $block . '' . $style. '" href="' .$link. '">' .do_shortcode($content). '</a>';
...
function sp_buttondd( $atts, $content = null ) {
    ...
    'isize' => '',
        ...
        $block = ($block == 'yes') ? "btn-block " : '';
        $size = ($size <> '') ? "btn-$size " : '';
        $isize = ($isize <> '') ? "icon-$isize" : '';
        $istyle = ($istyle <> '') ? "icon-$istyle" : '';

    $output = '
        <div class="btn-group">
            <a class="btn ' . $size . '' . $block . '' . $style. ' dropdown-toggle" data-toggle="dropdown" href="#"><i class="' . $istyle . ' ' . $isize. '"></i> ' .$label. ' <span class="caret"></span></a>
...
function sp_buttonsplit( $atts, $content = null ) {
    ...
    $output = '
        <div class="btn-group">
        <a ' . $target. ' class="btn ' . $size . '' . $block . '' . $style. '" href="'. $link .'"><i class="' . $istyle . ' ' . $isize. '"></i> ' .$label. ' </a><a class="btn ' . $size . '' . $style. ' dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></a>
            <ul class="dropdown-menu">
                '.do_shortcode($content).'
            </ul>
        </div>';
...
function sp_labels( $atts, $content = null ) {
    ...
    $out = '<span class="label label-' . $style . '">' .do_shortcode($content). '</span>';
...
function sp_badges( $atts, $content = null ) {
    ...
    $out = '<span class="badge badge-' . $style . '">' .do_shortcode($content). '</span>';
...
function sp_bars_cont( $atts, $content = null ) {
    extract(shortcode_atts(array(
    'width' => '',
    'strip' => '',
    'animate' => '',
...
add_shortcode('barc', 'sp_bars_cont');

//Single Progress Bars
function sp_bars_single( $atts, $content = null ) {
    extract(shortcode_atts(array(
    'width' => '',
    'style' => '',
    ), $atts));
...
add_shortcode('sbar', 'sp_bars_single');
